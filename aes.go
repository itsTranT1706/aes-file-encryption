package main

import (
	"fmt"
)

// SBox is the AES substitution box (S-box)
var sBox = [256]byte{
	,
}

// InvSBox is the inverse AES substitution box
var invSBox = [256]byte{
	,
}

// rcon contains the round constants for AES key expansion
var rcon = [...]byte{
	0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d,
}

// AES structure for configuration
type AES struct {
	Nb    int // Number of columns (32-bit words) comprising the State. For AES, Nb = 4.
	Nk    int // Number of 32-bit words comprising the Cipher Key. For AES-128, Nk = 4.
	Nr    int // Number of rounds. For AES-128, Nr = 10.
	w     [][]byte
	debug bool
}

func NewAES(key []byte, debug bool) *AES {
	nk := len(key) / 4
	nr := nk + 6
	a := &AES{
		Nb:    4,
		Nk:    nk,
		Nr:    nr,
		debug: debug,
	}
	a.w = a.keyExpansion(key)
	return a
}

func (a *AES) keyExpansion(key []byte) [][]byte {
	w := make([][]byte, a.Nb*(a.Nr+1))
	for i := 0; i < a.Nk; i++ {
		w[i] = []byte{key[4*i], key[4*i+1], key[4*i+2], key[4*i+3]}
	}

	for i := a.Nk; i < a.Nb*(a.Nr+1); i++ {
		temp := make([]byte, 4)
		copy(temp, w[i-1])

		if i%a.Nk == 0 {
			temp = a.subWord(a.rotWord(temp))
			temp[0] ^= rcon[i/a.Nk]
		} else if a.Nk > 6 && i%a.Nk == 4 {
			temp = a.subWord(temp)
		}

		w[i] = make([]byte, 4)
		for j := 0; j < 4; j++ {
			w[i][j] = w[i-a.Nk][j] ^ temp[j]
		}
	}
	return w
}

func (a *AES) rotWord(word []byte) []byte {
	return []byte{word[1], word[2], word[3], word[0]}
}

func (a *AES) subWord(word []byte) []byte {
	return []byte{sBox[word[0]], sBox[word[1]], sBox[word[2]], sBox[word[3]]}
}

func (a *AES) subBytes(state [][]byte) {
	for i := 0; i < 4; i++ {
		for j := 0; j < 4; j++ {
			state[i][j] = sBox[state[i][j]]
		}
	}
}

func (a *AES) invSubBytes(state [][]byte) {
	for i := 0; i < 4; i++ {
		for j := 0; j < 4; j++ {
			state[i][j] = invSBox[state[i][j]]
		}
	}
}

func (a *AES) shiftRows(state [][]byte) {
	// Row 1: Left shift by 1
	state[1][0], state[1][1], state[1][2], state[1][3] = state[1][1], state[1][2], state[1][3], state[1][0]
	// Row 2: Left shift by 2
	state[2][0], state[2][1], state[2][2], state[2][3] = state[2][2], state[2][3], state[2][0], state[2][1]
	// Row 3: Left shift by 3
	state[3][0], state[3][1], state[3][2], state[3][3] = state[3][3], state[3][0], state[3][1], state[3][2]
}

func (a *AES) invShiftRows(state [][]byte) {
	// Row 1: Right shift by 1
	state[1][0], state[1][1], state[1][2], state[1][3] = state[1][3], state[1][0], state[1][1], state[1][2]
	// Row 2: Right shift by 2
	state[2][0], state[2][1], state[2][2], state[2][3] = state[2][2], state[2][3], state[2][0], state[2][1]
	// Row 3: Right shift by 3
	state[3][0], state[3][1], state[3][2], state[3][3] = state[3][1], state[3][2], state[3][3], state[3][0]
}

func (a *AES) mixColumns(state [][]byte) {
	for c := 0; c < 4; c++ {
		s0, s1, s2, s3 := state[0][c], state[1][c], state[2][c], state[3][c]
		state[0][c] = mul(s0, 0x02) ^ mul(s1, 0x03) ^ s2 ^ s3
		state[1][c] = s0 ^ mul(s1, 0x02) ^ mul(s2, 0x03) ^ s3
		state[2][c] = s0 ^ s1 ^ mul(s2, 0x02) ^ mul(s3, 0x03)
		state[3][c] = mul(s0, 0x03) ^ s1 ^ s2 ^ mul(s3, 0x02)
	}
}

func (a *AES) invMixColumns(state [][]byte) {
	for c := 0; c < 4; c++ {
		s0, s1, s2, s3 := state[0][c], state[1][c], state[2][c], state[3][c]
		state[0][c] = mul(s0, 0x0e) ^ mul(s1, 0x0b) ^ mul(s2, 0x0d) ^ mul(s3, 0x09)
		state[1][c] = mul(s0, 0x09) ^ mul(s1, 0x0e) ^ mul(s2, 0x0b) ^ mul(s3, 0x0d)
		state[2][c] = mul(s0, 0x0d) ^ mul(s1, 0x09) ^ mul(s2, 0x0e) ^ mul(s3, 0x0b)
		state[3][c] = mul(s0, 0x0b) ^ mul(s1, 0x0d) ^ mul(s2, 0x09) ^ mul(s3, 0x0e)
	}
}

func (a *AES) addRoundKey(state [][]byte, round int) {
	for c := 0; c < 4; c++ {
		keyWord := a.w[round*a.Nb+c]
		for r := 0; r < 4; r++ {
			state[r][c] ^= keyWord[r]
		}
	}
}

func mul(a, b byte) byte {
	var p byte = 0
	for i := 0; i < 8; i++ {
		if (b & 1) != 0 {
			p ^= a
		}
		hiBitSet := (a & 0x80) != 0
		a <<= 1
		if hiBitSet {
			a ^= 0x1b
		}
		b >>= 1
	}
	return p
}

func (a *AES) Encrypt(block []byte) []byte {
	state := make([][]byte, 4)
	for i := 0; i < 4; i++ {
		state[i] = make([]byte, 4)
		for j := 0; j < 4; j++ {
			state[i][j] = block[i+4*j]
		}
	}

	if a.debug {
		fmt.Println("Round 0: Initial State Matrix")
		printState(state)
	}

	a.addRoundKey(state, 0)
	if a.debug {
		fmt.Println("Round 0 AddRoundKey:")
		printState(state)
	}

	for round := 1; round < a.Nr; round++ {
		if a.debug {
			fmt.Printf("\n--- Round %d ---\n", round)
		}
		a.subBytes(state)
		if a.debug {
			fmt.Println("SubBytes:")
			printState(state)
		}
		a.shiftRows(state)
		if a.debug {
			fmt.Println("ShiftRows:")
			printState(state)
		}
		a.mixColumns(state)
		if a.debug {
			fmt.Println("MixColumns:")
			printState(state)
		}
		a.addRoundKey(state, round)
		if a.debug {
			fmt.Println("AddRoundKey:")
			printState(state)
		}
	}

	// Final round
	if a.debug {
		fmt.Printf("\n--- Final Round %d ---\n", a.Nr)
	}
	a.subBytes(state)
	if a.debug {
		fmt.Println("SubBytes:")
		printState(state)
	}
	a.shiftRows(state)
	if a.debug {
		fmt.Println("ShiftRows:")
		printState(state)
	}
	a.addRoundKey(state, a.Nr)
	if a.debug {
		fmt.Println("Final State Matrix (Ciphertext):")
		printState(state)
	}

	res := make([]byte, 16)
	for i := 0; i < 4; i++ {
		for j := 0; j < 4; j++ {
			res[i+4*j] = state[i][j]
		}
	}
	return res
}

func (a *AES) Decrypt(block []byte) []byte {
	state := make([][]byte, 4)
	for i := 0; i < 4; i++ {
		state[i] = make([]byte, 4)
		for j := 0; j < 4; j++ {
			state[i][j] = block[i+4*j]
		}
	}

	if a.debug {
		fmt.Println("Round 0: Initial State Matrix (Ciphertext)")
		printState(state)
	}

	a.addRoundKey(state, a.Nr)
	if a.debug {
		fmt.Printf("Round 0 AddRoundKey (Key %d):\n", a.Nr)
		printState(state)
	}

	for round := a.Nr - 1; round >= 1; round-- {
		if a.debug {
			fmt.Printf("\n--- Round %d ---\n", a.Nr-round)
		}
		a.invShiftRows(state)
		if a.debug {
			fmt.Println("InvShiftRows:")
			printState(state)
		}
		a.invSubBytes(state)
		if a.debug {
			fmt.Println("InvSubBytes:")
			printState(state)
		}
		a.addRoundKey(state, round)
		if a.debug {
			fmt.Printf("AddRoundKey (Key %d):\n", round)
			printState(state)
		}
		a.invMixColumns(state)
		if a.debug {
			fmt.Println("InvMixColumns:")
			printState(state)
		}
	}

	// Final round
	if a.debug {
		fmt.Printf("\n--- Final Round %d ---\n", a.Nr)
	}
	a.invShiftRows(state)
	if a.debug {
		fmt.Println("InvShiftRows:")
		printState(state)
	}
	a.invSubBytes(state)
	if a.debug {
		fmt.Println("InvSubBytes:")
		printState(state)
	}
	a.addRoundKey(state, 0)
	if a.debug {
		fmt.Println("Final State Matrix (Plaintext):")
		printState(state)
	}

	res := make([]byte, 16)
	for i := 0; i < 4; i++ {
		for j := 0; j < 4; j++ {
			res[i+4*j] = state[i][j]
		}
	}
	return res
}

func printState(state [][]byte) {
	for i := 0; i < 4; i++ {
		fmt.Printf("[ ")
		for j := 0; j < 4; j++ {
			fmt.Printf("%02x ", state[i][j])
		}
		fmt.Printf("]\n")
	}
}
